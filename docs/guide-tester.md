...
目前是 **9 個檔案、66 個 `@test`**（`grep -c '^@test' bin/test/*.bats` 可自行核對）。
...
> **`--quiet` 目前只被解析，沒有被任何地方讀取**（`bin/cmd/verify.sh` 裡
> `quiet=1` 賦值之後不再出現）。加了它輸出不會變少。這裡照實寫，
> 因為一個測試者若以為它會改變輸出，會誤判「這次跑的範圍比較少」。
