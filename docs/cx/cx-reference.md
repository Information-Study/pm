...
**測試與掃描只在 `test` 模式出現，部署只在 `prod` 模式出現。**
標題列會說明其餘的在哪個模式（藏起來卻不說去哪裡找，使用者只會以為壞了）。

| 項目 | dev | test | prod |
|---|---|---|---|
| 現況一覽 / 切換模式 / 專案 / 環境 / 容器 / 工具 / Git / 自訂 / 說明 | ✔ | ✔ | ✔ |
| 測試・DevSecOps | | ✔ | |
| 部署 | | | ✔ |

> ⚠ **這件事讓 `TUI-coverage` 檢查的語意變了，而那個變化差點是隱形的。**
> `verify_meta.py` 的 `check_tui` 是**靜態 regex 剖析**，它看不到 `$_TUI_MODE`。
> 把選單包進 `if [[ $_TUI_MODE == test ]]` 之後，regex 照樣抓得到那些行，
> 檢查會**照樣 PASS —— 但不再證明任何事**：它會宣稱「每個動詞都到得了」，
> 而 dev 模式下 scan 一個都到不了。那比檢查變紅糟得多。
>
> 所以 `check_tui` 改成**對三個模式各走訪一次選單圖**，斷言聯集涵蓋所有
> 非豁免動詞，並把每個模式各能到幾個印在備註裡：
>
> ```
> ✔ TUI-coverage 三個模式的聯集涵蓋每個動詞
>     聯集 33 個・各模式可達：dev 24・test 26・prod 24・13 個刻意只走命令列
> ```
>
> 模式門檻用 `tui.sh` 裡的 `# @tui-mode: <清單>` 標記表示 ——
> **刻意不去剖析 bash 的條件式**：那等於新增一個會漂移的剖析器。
> 標記整個消失時檢查會 FAIL（否則它會安靜地退回舊語意）。
> `bin/test/80_verify.bats` 有兩條反向案例釘住這兩種失效。

> ⚠ **選單項目必須在 `while` 迴圈「內」重建。**
> 第一版把 `items` 陣列建在迴圈外，只算一次 —— 於是在選單裡切換模式之後
> 項目完全不變。這個缺陷的外觀特別容易被當成「切換沒有作用」：
> 標題字串是在 `_tui_menu` 的**參數位置**求值的，每次迭代都會重算，
> 所以「[模式：test]」**會**跟著變，只有底下的項目沒變 ——
> 畫面上是一個說自己在 test 模式、卻沒有測試與掃描的選單。
>
...
`init` / `re-init` / `rename` / 抹除紀錄 / 現況一覽 收在「專案」底下。
它們改的是**這個專案是誰**；「環境」改的是**這台機器能不能跑它**。
兩件不同的事，原本混在同一個選單裡。

### 容器：Volume 與 Image

`容器` 選單底下多了兩項。它們回答的是兩個常見問題：

* **Volume** —— 「為什麼硬碟滿了」。只列**這個專案的**具名 volume 與佔用空間。
  刪除走 `cx_confirm --danger`（紅線 2），而且一次一個。
  刻意**不做** `docker volume prune` 那種全域清理 —— 那會掃到別的專案，
  應該是有意識的決定，不該藏在選單裡。
* **Image** —— 「為什麼改了 Dockerfile 卻沒生效」。列出這個專案建出來的映像，
  並附上答案：`up` 沒加 `--build` 就會用既有的映像；而映像 tag 一定含模式，
  因為 spa / static 的 API base URL 是 build 時烘進 bundle 的。
