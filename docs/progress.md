...
| `test cli` | ✅ | `cx` 自己的行為測試（bats-core，**163 個案例**）。bats 把 skip 算成成功，與本專案 SKIP≠PASS 的教條衝突，所以 `_test_cli` 會另外把跳過數印出來，並支援 `CX_TEST_STRICT=1` |
...
`reports/`、`.cx/`、ansible 的 collections 目錄全部 `rm -rf` 之後（共 1.9 GB）：
